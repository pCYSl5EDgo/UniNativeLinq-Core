...
namespace CecilRewrite
{
    ...
    static class IntersectExceptDefaultComparerHelper
    {
        internal static void Create(ModuleDefinition module)
        {
            ...
            {
                Make(IntersectDefaultComparerHelper, type0, "Array", 0, CalcArrayType, AsRefEnumerableArray, nameof(Intersect), Intersect);
                Make(IntersectDefaultComparerHelper, type0, "Array", 1, CalcArrayType, AsRefEnumerableArray, nameof(Intersect), Intersect);
                Make(ExceptDefaultComparerHelper, type0, "Native", 0, NativeArray.MakeGenericInstanceType, AsRefEnumerableNative, nameof(Except), Except);
                Make(ExceptDefaultComparerHelper, type0, "Native", 1, NativeArray.MakeGenericInstanceType, AsRefEnumerableNative, nameof(Except), Except);
        ...
        {
            var MainModule = Prologue(@static, type, name, out var method, out var T, out var DefaultOrderByAscending, out var Enumerable0, out var Enumerator0, out var Element0);
            ...
            var OperatorGenericArguments = new[]
            {
                Enumerable0,
                Enumerator0,
                Enumerable0,
            ...
            OperatorGenericArguments[index << 1] = Enumerable;
            OperatorGenericArguments[1 + (index << 1)] = Enumerator;
            var Operation = operation.MakeGenericInstanceType(OperatorGenericArguments);
            ...
            ReturnGenericArguments[1 + (index << 1)] = Enumerator;
            ...
            variables.Add(new VariableDefinition(Operation));
        ...
        {
            var MainModule = Prologue(@static, type0, name, out var method, out var T, out var DefaultOrderByAscending, out var Enumerable0, out var Enumerator0, out var Element0);
        ...
        private static ModuleDefinition Prologue(TypeDefinition @static, TypeDefinition type0, string name, out MethodDefinition method, out GenericParameter T, out GenericInstanceType DefaultOrderByAscending, out GenericInstanceType Enumerable0, out TypeReference Enumerator0, out TypeReference Element0)
        {
            ...
            DefaultOrderByAscending = MainModule.GetType(NameSpace, "DefaultOrderByAscending`1").MakeGenericInstanceType(T);
