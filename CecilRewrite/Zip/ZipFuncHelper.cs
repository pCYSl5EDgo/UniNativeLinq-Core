...
namespace CecilRewrite
{
    ...
    static class ZipFuncHelper
    {
        internal static void Create(ModuleDefinition module)
        {
            TypeDefinition Adjusted = new TypeDefinition(NameSpace,
                nameof(Adjusted) + nameof(ZipFuncHelper),
            ...
            TypeDefinition Exceptional = new TypeDefinition(NameSpace,
                nameof(Exceptional) + nameof(ZipFuncHelper),
                StaticExtensionClassTypeAttributes, module.TypeSystem.Object);
        ...
        private static void Zip(TypeDefinition @static, TypeDefinition type, string prefix, Func<IEnumerable<TypeReference>, TypeReference> InputFunc, MethodDefinition constructor, string typePrefix, int index)
        {
            ...
            TypeReference TAction, TActionFunc;
            GenericInstanceType @return;
            if (index == 0)
                Epilogue(method, MainModule, typePrefix, EnumerableX, EnumeratorX, ElementX, Enumerable, Enumerator, Element, out @return, out _, out TAction, out TActionFunc);
            else
                Epilogue(method, MainModule, typePrefix, Enumerable, Enumerator, Element, EnumerableX, EnumeratorX, ElementX, out @return, out _, out TAction, out TActionFunc);
            ...
            ParameterDefinition action = new ParameterDefinition(nameof(action), ParameterAttributes.None, TActionFunc);
            method.Parameters.Add(action);
        ...
        private static void Zip(TypeDefinition @static, TypeDefinition type0, TypeDefinition type1, string typePrefix)
        {
            ...
            Epilogue(method, MainModule, typePrefix, Enumerable0, Enumerator0, Element0, Enumerable1, Enumerator1, Element1, out var @return, out var T, out var TAction, out var TActionFunc);
        ...
        private static void Epilogue(MethodDefinition method, ModuleDefinition MainModule, string typePrefix, GenericInstanceType enumerable0, TypeReference enumerator0, TypeReference element0, GenericInstanceType enumerable1, TypeReference enumerator1, TypeReference element1, out GenericInstanceType @return, out TypeReference T, out TypeReference TAction, out TypeReference TActionFunc)
        {
            var Tuple2 = new[] { element0, element1 };
            var _T = new GenericParameter(nameof(T), method) { HasNotNullableValueTypeConstraint = true };
            _T.CustomAttributes.Add(UnManagedAttribute);
            method.GenericParameters.Add(_T);
            T = _T;

            var TActionTuple3 = new[]
            {
                element0,
                element1,
                T,
            };
            TAction = MainModule.GetType(NameSpace, "DelegateFuncToStructOperatorAction`3").MakeGenericInstanceType(TActionTuple3);
            TActionFunc = MainModule.ImportReference(SystemModule.GetType("System", "Func`3")).MakeGenericInstanceType(TActionTuple3);
