...
namespace CecilRewrite
{
    ...
    static class OrderByRefFuncHelper
    {
        internal static void Create(ModuleDefinition module)
        {
            var @static = new TypeDefinition(NameSpace,
                nameof(OrderByRefFuncHelper),
        ...
        private static void OrderBy(TypeDefinition @static, TypeDefinition type)
        {
            ...
            var TPredicate0Func = MainModule.GetType(NameSpace, "RefFunc`3").MakeGenericInstanceType(TPredicate0Tuple);
            var TPredicate0 = MainModule.GetType(NameSpace, "DelegateRefFuncToStructOperatorFunc`3").MakeGenericInstanceType(TPredicate0Tuple);
