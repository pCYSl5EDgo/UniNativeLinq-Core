...
namespace CecilRewrite
{
    ...
    static class DistinctRefFuncHelper
    {
        internal static void Create(ModuleDefinition module)
        {
            TypeDefinition @static;
            @static = new TypeDefinition(NameSpace,
                nameof(DistinctRefFuncHelper),
        ...
        private static void Distinct(TypeDefinition @static, TypeDefinition type)
        {
            ...
            TEqualityComparer0 = MainModule.GetType(NameSpace, "DelegateRefFuncToStructOperatorFunc`3").MakeGenericInstanceType(EqualityTuple);
            TEqualityComparer0Func = MainModule.GetType(NameSpace, "RefFunc`3").MakeGenericInstanceType(EqualityTuple);
            TGetHashCodeFunc0 = MainModule.GetType(NameSpace, "DelegateRefFuncToStructOperatorFunc`2").MakeGenericInstanceType(GetHashCodeTuple);
            TGetHashCodeFunc0Func = MainModule.GetType(NameSpace, "RefFunc`2").MakeGenericInstanceType(GetHashCodeTuple);
